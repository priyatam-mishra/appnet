import React from 'react'
import './page1.css'
export default function amazonmtv() {
    return (
        <>
            <form action="">
                <div className="ele1">
                    <div className="elea1">
                        <h5>category</h5>
                        <h6> {'<'} Electronics</h6>
                        <ul>
                            <h4>Mobiles & Accessories</h4>
                            <li>
                                <h6>Mobile Accessories
                                </h6>
                            </li>
                            <li>
                                <h6>
                                    Mobile Broadband Devices

                                </h6>
                            </li>
                            <li>
                                <h6>
                                    SIM Cards
                                </h6>

                            </li>
                            <li>
                                <h6>
                                    Smartphones & Basic

                                </h6>
                            </li>
                            <li>
                                <h6>
                                    Mobiles

                                </h6>
                            </li>
                            <li>
                                <h6>    Smartwatches  </h6>
                            </li>
                        </ul>
                        <ul>
                            <h4>Made for Amazon Brands</h4>
                            <li>

                                <input type="radio" name="" id="" />
                                made for Amazon

                            </li>
                        </ul>
                        <ul>
                            <h4>Amazon prime</h4>
                            <li className='eli1'>

                                <input type="radio" name="" id="" />
                                <span>✓
                                </span>prime

                            </li>
                        </ul>
                        <ul>
                            <h4>delivery day</h4>
                            <li>
                                <input type="radio" name="" id="" />
                                get it by tomorrow

                            </li>
                        </ul>
                        <ul>
                            <h4>pay on Delivery</h4>
                            <li>

                                <input type="radio" name="" id="" />
                                Eligible for pay on <br />......Delivery

                            </li>
                        </ul>
                        <ul>
                            <h4>Brand</h4>
                            <li>
                                <form action="">
                                    <input type="radio" name="" id="" />
                                    iQOO <br />
                                    <input type="radio" name="" id="" />
                                    Apple <br />
                                    <input type="radio" name="" id="" />
                                    OnePlus <br />
                                    <input type="radio" name="" id="" />
                                    realme <br />
                                    <input type="radio" name="" id="" />
                                    Samsung <br />
                                    <input type="radio" name="" id="" />
                                    Fire-boltt <br />
                                    <input type="radio" name="" id="" />
                                    Ambrane
                                </form>
                            </li>
                        </ul>
                        <ul>
                            <h4>new Arrivals</h4>
                            <li> last 30 days</li>
                            <li>  last 90 days</li>
                        </ul>
                        <ul>
                            <h4>item condition</h4>
                            <li> new</li>
                            <li> renewed</li>
                            <li> used</li>
                        </ul>
                        <ul>
                            <h4>price</h4>
                            <li>under ₹1,000</li>
                            <li>₹1,000-₹5,000</li>
                            <li>₹5,000-₹10,000</li>
                            <li>
                                ₹10,000-₹20,000
                            </li>

                            <li>
                                over ₹20,000
                            </li>
                        </ul>
                        <ul>
                            <h4>Deal & Discounts</h4>
                            <li>all Discounts</li>
                            <li>today's Deal</li>
                        </ul>
                        <ul>
                            <h4>Discount</h4>
                            <li>10% off or more</li>
                            <li>20% off or more</li>
                            <li>30% off or more</li>
                            <li>40% off or more</li>
                            <li>50% off or more</li>
                            <li>60% off or more</li>

                        </ul>
                        <ul>
                            <h4>seller</h4>
                            <li><input type="radio" name="" id="" />
                                COCObLu Retail

                            </li>
                            <li><input type="radio" name="" id="" /> Appario Retail Private Ltd</li>
                            <li><input type="radio" name="" id="" />
                                Spigen India

                            </li>

                            <li><input type="radio" name="" id="" />
                                Coberta Protective

                            </li>
                        </ul>
                        <ul>
                            <h4>Accessories</h4>
                            <li><input type="radio" name="" id="" />
                                Stylabs
                            </li>
                            <li><input type="radio" name="" id="" />  ACCESSORIESMASTERS
                            </li>
                            <li><input type="radio" name="" id="" /> Easy Cart
                            </li>
                            <li><input type="radio" name="" id="" />  VONZEEO
                            </li>
                            <li><input type="radio" name="" id="" />  Best Quality Online Seller
                            </li>
                            <li><input type="radio" name="" id="" /> GlassVerse  </li>
                        </ul>
                        <ul>
                            <h4>Availability</h4>
                            <li><input type="radio" name="" id="" />
                                Include Out of Stock
                            </li>
                        </ul>
                    </div>
                    <div className="elea2"></div>
                </div>
            </form>
            <div className="ele2">
                <div className="eleb1">
                    <div className="eleba1">
                        <h1>Smartphone EMI carnival</h1>
                        <h2>23 <sup>rd</sup>-27 <sup>th</sup> February </h2>
                        <h2>Up to 18 no cost EMI</h2>
                        <span>
                            visit the no cast EMI store now
                        </span>
                    </div>

                </div>
                <div className="eleb2">
                    <div className="elebb1">
                        <div className="bb">

                            <h4>OnePlus 12R</h4>
                            <h6>pristine display</h6>
                            <h6>with aqua touch</h6>
                        </div>
                        <span>starting <sup>₹</sup>4,333/month*</span>

                    </div>
                    <div className="elebb1">     <div className="bb">

                        <h4>OnePlus 12R</h4>
                        <h6>pristine display</h6>
                        <h6>with aqua touch</h6>
                    </div>
                        <span>starting <sup>₹</sup>4,333/month*</span>

                    </div>
                    <div className="elebb1">
                        <div className="bb">

                            <h4>OnePlus 12R</h4>
                            <h6>pristine display</h6>
                            <h6>with aqua touch</h6>
                        </div>
                        <span>starting <sup>₹</sup>4,333/month*</span>

                    </div>
                    <div className="elebb1">
                        <div className="bb">

                            <h4>OnePlus 12R</h4>
                            <h6>pristine display</h6>
                            <h6>with aqua touch</h6>
                        </div>
                        <span>starting <sup>₹</sup>4,333/month*</span>
                    </div>
                    <div className="elebb1">
                        <div className="bb">

                            <h4>OnePlus 12R</h4>
                            <h6>pristine display</h6>
                            <h6>with aqua touch</h6>
                        </div>
                        <span>starting <sup>₹</sup>4,333/month*</span>

                    </div>
                    <div className="elebb1">
                        <div className="bb">

                            <h4>OnePlus 12R</h4>
                            <h6>pristine display</h6>
                            <h6>with aqua touch</h6>
                        </div>
                        <span>starting <sup>₹</sup>4,333/month*</span>

                    </div>
                </div>
                <div className="eleb3">
                    <div className="elebc1">
                        <div className="elebc2">
                            <div className="cc">
                                <h4>realme</h4>
                                <h5>NARZO</h5>
                            </div>
                            <div className="cc2">
                                <h2>amazonspecials</h2>
                            </div>
                        </div>
                        <div className="cc4"></div>
                        <div className="elebc3">
                            <div className="cc3">
                                <h2>NARZO 70 Pro 5G</h2>
                                <h3>Elevate every shot</h3>
                                <h6>See in March</h6>

                            </div>
                        </div>
                    </div>

                </div>
                <div className="eleb2 bt">
                    <div className="elebb1">
                        <div className="bb">

                            <h4>OnePlus 24R</h4>
                            <h6>pristine display</h6>
                            <h6>with aqua touch</h6>
                        </div>
                        <span>shop now</span>

                    </div>
                    <div className="elebb1">     <div className="bb">

                        <h4>OnePlus 24R</h4>
                        <h6>pristine display</h6>
                        <h6>with aqua touch</h6>
                    </div>
                        <span>shop now</span>

                    </div>
                    <div className="elebb1">
                        <div className="bb">

                            <h4>OnePlus 24R</h4>
                            <h6>pristine display</h6>
                            <h6>with aqua touch</h6>
                        </div>
                        <span>shop now</span>

                    </div>
                    <div className="elebb1">
                        <div className="bb">

                            <h4>OnePlus 24R</h4>
                            <h6>pristine display</h6>
                            <h6>with aqua touch</h6>
                        </div>
                        <span>shop now</span>
                    </div>
                    <div className="elebb1">
                        <div className="bb">

                            <h4>OnePlus 24R</h4>
                            <h6>pristine display</h6>
                            <h6>with aqua touch</h6>
                        </div>
                        <span>shop now</span>

                    </div>
                    <div className="elebb1">
                        <div className="bb">

                            <h4>OnePlus 24R</h4>
                            <h6>pristine display</h6>
                            <h6>with aqua touch</h6>
                        </div>
                        <span >shop now</span>

                    </div>
                </div>
                <div className="eleb4">
                    <div className="elebd1">
                        <div className="bd1">
                            <h4>citi</h4>
                        </div>
                        <div className="bd2">
                            <h5>10% instant discount*</h5>
                            <h6>on Credit/Debit Cards & Credit EMI</h6>
                        </div>
                    </div>
                    <div className="elebd3"></div>
                    <div className="elebd2">
                        <h6>This product is issued by Axis Bank. using certain tradernarks  temporarily under license from Citiyoto  lnc. and related group entities.</h6>
                    </div>
                </div>
                <div className="eleb5">
                    <div className="bn1">
                        <div className="bq1 bq">
                            <div className="_">
                                <h6>oneplus</h6>
                                <h6>12r</h6>
                                <span>
                                    <h6><sup>₹</sup>39,999</h6>
                                    <div className="sf">
                                        <h5>effective price</h5>
                                        <h6><sup>₹</sup>38,999*</h6>
                                    </div>
                                </span>
                                <h5>5500mAh battery</h5>
                                <h5>snapdragon 8</h5>
                                <h5>Gen 2 processor</h5>
                            </div>
                            <div className="__"></div>
                        </div>
                        <div className="bq2 bq">
                            <div className="_">
                                <h6>oneplus</h6>
                                <h6>12r</h6>
                                <span>
                                    <h6><sup>₹</sup>39,999</h6>
                                    <div className="sf">
                                        <h5>effective price</h5>
                                        <h6><sup>₹</sup>38,999*</h6>
                                    </div>
                                </span>
                                <h5>5500mAh battery</h5>
                                <h5>snapdragon 8</h5>
                                <h5>Gen 2 processor</h5>
                            </div>
                            <div className="__"></div>
                        </div>
                        <div className="bq3 bq">
                            <div className="_">
                                <h6>oneplus</h6>
                                <h6>12r</h6>
                                <span>
                                    <h6><sup>₹</sup>39,999</h6>
                                    <div className="sf">
                                        <h5>effective price</h5>
                                        <h6><sup>₹</sup>38,999*</h6>
                                    </div>
                                </span>
                                <h5>5500mAh battery</h5>
                                <h5>snapdragon 8</h5>
                                <h5>Gen 2 processor</h5>
                            </div>
                            <div className="__"></div>
                        </div>
                        <div className="bq4 bq">
                            <div className="_">
                                <h6>oneplus</h6>
                                <h6>12r</h6>
                                <span>
                                    <h6><sup>₹</sup>39,999</h6>
                                    <div className="sf">
                                        <h5>effective price</h5>
                                        <h6><sup>₹</sup>38,999*</h6>
                                    </div>
                                </span>
                                <h5>5500mAh battery</h5>
                                <h5>snapdragon 8</h5>
                                <h5>Gen 2 processor</h5>
                            </div>
                            <div className="__"></div>
                        </div>
                        <div className="bq5 bq">
                            <div className="_">
                                <h6>oneplus</h6>
                                <h6>12r</h6>
                                <span>
                                    <h6><sup>₹</sup>39,999</h6>
                                    <div className="sf">
                                        <h5>effective price</h5>
                                        <h6><sup>₹</sup>38,999*</h6>
                                    </div>
                                </span>
                                <h5>5500mAh battery</h5>
                                <h5>snapdragon 8</h5>
                                <h5>Gen 2 processor</h5>
                            </div>
                            <div className="__"></div>
                        </div>
                    </div>
                    <div className="bn2">
                        <div className="bq6 bq">
                            <div className="_">
                                <h6>oneplus</h6>
                                <h6>12r</h6>
                                <span>
                                    <h6><sup>₹</sup>39,999</h6>
                                    <div className="sf">
                                        <h5>effective price</h5>
                                        <h6><sup>₹</sup>38,999*</h6>
                                    </div>
                                </span>
                                <h5>5500mAh battery</h5>
                                <h5>snapdragon 8</h5>
                                <h5>Gen 2 processor</h5>
                            </div>
                            <div className="__"></div>
                        </div>
                        <div className="bq7 bq">
                            <div className="_">
                                <h6>oneplus</h6>
                                <h6>12r</h6>
                                <span>
                                    <h6><sup>₹</sup>39,999</h6>
                                    <div className="sf">
                                        <h5>effective price</h5>
                                        <h6><sup>₹</sup>38,999*</h6>
                                    </div>
                                </span>
                                <h5>5500mAh battery</h5>
                                <h5>snapdragon 8</h5>
                                <h5>Gen 2 processor</h5>
                            </div>
                            <div className="__"></div>
                        </div>
                        <div className="bq8 bq">
                            <div className="_">
                                <h6>oneplus</h6>
                                <h6>12r</h6>
                                <span>
                                    <h6><sup>₹</sup>39,999</h6>
                                    <div className="sf">
                                        <h5>effective price</h5>
                                        <h6><sup>₹</sup>38,999*</h6>
                                    </div>
                                </span>
                                <h5>5500mAh battery</h5>
                                <h5>snapdragon 8</h5>
                                <h5>Gen 2 processor</h5>
                            </div>
                            <div className="__"></div>
                        </div>
                        <div className="bq9 bq">
                            <div className="_">
                                <h6>oneplus</h6>
                                <h6>12r</h6>
                                <span>
                                    <h6><sup>₹</sup>39,999</h6>
                                    <div className="sf">
                                        <h5>effective price</h5>
                                        <h6><sup>₹</sup>38,999*</h6>
                                    </div>
                                </span>
                                <h5>5500mAh battery</h5>
                                <h5>snapdragon 8</h5>
                                <h5>Gen 2 processor</h5>
                            </div>
                            <div className="__"></div>
                        </div>
                        <div className="bq10 bq">
                            <div className="_">
                                <h6>oneplus</h6>
                                <h6>12r</h6>
                                <span>
                                    <h6><sup>₹</sup>39,999</h6>
                                    <div className="sf">
                                        <h5>effective price</h5>
                                        <h6><sup>₹</sup>38,999*</h6>
                                    </div>
                                </span>
                                <h5>5500mAh battery</h5>
                                <h5>snapdragon 8</h5>
                                <h5>Gen 2 processor</h5>
                            </div>
                            <div className="__"></div>
                        </div>
                    </div>

                </div>
            </div>
            <div className="ele3">
                <div className="cc8">
                    <div className="c10">
                        <div className="btn btn-primary"> back to top</div>
                    </div>
                    <div className="c11">
                        <ul className="ul1">
                            <h3>Get to Know Us</h3>
                            <li>About us</li>
                            <li>Careers</li>
                            <li>Press Releases</li>
                            <li>Atnazon Science</li>
                        </ul>
                        <ul className="ul2">
                            <h3>Connect with Us</h3>
                            <li>Facebook </li>
                            <li>Twitter</li>
                            <li>Instagram</li>
                        </ul>
                        <ul className="ul3">
                            <h3>Make Money with Us</h3>
                            <li>
                                sell on Arnazon
                            </li>
                            <li>sell under amazon accelerator</li>
                            <li>portect and build your brand</li>
                            <li>amzazon global selling</li>
                            <li>become an affiliate</li>
                            <li>fulfilment by amazon</li>
                            <li>advertise your products</li>
                            <li>amazon pay on merchants</li>
                        </ul>
                        <ul className="ul1">
                            <h3>let us help you</h3>
                            <li>COVID-19 and Amazon</li>
                            <li>Your Account </li>
                            <li>  Peturns Centre</li>
                            <li>100% Purchase Protection</li>
                            <li>Amazon Agp Download</li>
                            <li>help</li>
                        </ul>

                    </div>
                    <div className="c12">
                        <div className="imf"></div>
                        <select name="" id="">
                            <option value="">
                                English
                            </option>
                        </select>
                    </div>
                    <div className="c13">
                        <ul>
                            <li>Australia</li>
                            <li> Brazil</li>
                            <li>Canada</li>
                            <li>China </li>
                            <li>France</li>
                            <li> Germany</li>
                            <li> Italy</li>
                            <li>Japan</li>
                            <li> Mecico</li>
                            <li>Netherlands</li>
                            <li> Poland </li>
                            <li> Singapore</li>
                            <li>Spain </li>
                            <li> Turkey</li>
                            <li> United Arab Emirates</li>
                            <li >United Kingdom</li>
                            <li className='le1'>United States</li>

                        </ul>

                    </div>
                </div>
                <div className="cc9">
                    <div className="c14">
                        <div className="ce1">
                            <ul>
                                <h4>AbeBooks</h4>
                                <li>books,art</li>
                                <li>& collectibles</li>
                            </ul>
                        </div>

                        <div className="ce2">
                            <ul>
                                <h4>Shopbop</h4>
                                <li>Designer</li>
                                <li>Fashion brands </li>
                            </ul>



                        </div>
                        <div className="ce3">
                            <ul>
                                <h4>Amazon web services</h4>
                                <li>scalable cloud</li>
                                <li>computing services</li>
                            </ul>
                        </div>
                        <div className="ce4">
                            <ul>
                                <h4>Audible</h4>
                                <li>Download</li>
                                <li>Audio books</li>
                            </ul>
                        </div>
                    </div>
                    <div className="c15">
                        <div className="ce5">
                            <ul>
                                <h4>IMDb</h4>
                                <li>Movies,TV</li>
                                <li>& celebrities</li>
                            </ul>
                        </div>
                        <div className="ce6">
                            <ul>
                                <h4>Amazon Business</h4>
                                <li> Everything For</li>
                                <li>Your Business</li>
                            </ul>
                        </div>
                        <div className="ce7"> <ul>
                            <h4>Prime Now</h4>
                            <li>2-Hour Delivery</li>
                            <li>on Everyday Items</li>
                        </ul>
                        </div>
                        <div className="ce8">
                            <ul>
                                <h4>Amazon Prime Music</h4>
                                <li> 100 million songs, ad-free</li>
                                <li>Over IS million podcast episodes</li>
                            </ul>
                        </div>
                    </div>
                    <div className="c17">
                        <ul>
                            <li>Conditions of use & Sale</li>
                            <li>Privacy Notice</li>
                            <li>Interest-eased Ads</li>
                        </ul>
                        <ul>
                            <li>© 01996-2024, Amazon.com, Inc. or its affiliates</li>
                        </ul>
                    </div>
                </div>
            </div>
        </>
    )

}
